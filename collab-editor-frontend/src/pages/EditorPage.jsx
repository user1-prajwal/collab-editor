...
function EditorPage() {
        ...
        [
          {
            ...
            options: { beforeContentClassName: className, stickiness: 1 },
          },
        ],
      );
    });
    Object.keys(decorationsRef.current).forEach((name) => {
      if (!cursors[name]) {
        ...
        const styleEl = document.getElementById(
          `style-remote-cursor-${name.replace(/\s/g, "-")}`,
        );
  ...
  function handleCodeChange(value) {
    ...
    if (wsRef.current?.readyState === 1) {
      wsRef.current.send(
        JSON.stringify({ type: "code", code: value, fileId: activeFileId }),
      );
    }
  }
    ...
    setFiles((prev) =>
      prev.map((f) => {
        ...
        return {
          ...f,
          language: newLang,
          name: `${baseName}.${extMap[newLang] || "txt"}`,
        };
      }),
    );
    if (wsRef.current?.readyState === 1) {
  ...
  async function executeCode() {
    ...
    if (activeFile.language === "javascript") {
      try {
        ...
        window.print = () => {
          result += "⚠️ Use console.log() not print() in JavaScript!\n";
        };
        console.log = (...args) => {
          result += args.join(" ") + "\n";
        };
  ...
  function importFile() {
    ...
    input.multiple = true;
    input.accept =
      ".py,.java,.cpp,.ts,.txt,.html,.css,.json,.c,.cs,.fs,.php,.rb,.hs,.go,.rs";
    input.onchange = (e) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          <p style={{ color: "#888", margin: "0 0 10px", fontSize: "13px" }}>
            OUTPUT
          </p>
          <pre
            style={{
