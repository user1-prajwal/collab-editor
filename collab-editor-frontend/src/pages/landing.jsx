...
function Navbar({ onOpen }) {
  ...
  return (
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, height: '60px', display: 'flex', alignItems: 'center', padding: '0 32px', justifyContent: 'space-between', background: pinned ? 'rgba(9,9,11,0.92)' : 'transparent', backdropFilter: pinned ? 'blur(12px)' : 'none', borderBottom: pinned ? '1px solid rgba(255,255,255,0.06)' : '1px solid transparent', transition: 'all 0.25s' }}>
      ...
      <button className="ce-btn-primary" onClick={onOpen} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 18px', background: '#6366F1', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter,sans-serif' }}>
...
function Hero({ onOpen }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="hero-grid" style={{ flex: 1, display: 'flex', alignItems: 'center', maxWidth: '1160px', margin: '0 auto', width: '100%', padding: '110px 32px 40px', gap: '56px' }}>
        {/* LEFT */}
        <div className="hero-left" style={{ flex: '0 0 auto', width: '480px', animation: 'fadeIn 0.5s ease both' }}>
          <h1 style={{ fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 900, lineHeight: 1.06, letterSpacing: '-2px', color: '#F8FAFC', marginBottom: '20px' }}>
            Code together,<br />
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '14px' }}>
            <button className="ce-btn-primary" onClick={onOpen} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '13px 28px', background: '#6366F1', border: 'none', borderRadius: '9px', color: '#fff', fontSize: '15px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter,sans-serif' }}>
               Create a room
            </button>
            <div style={{ display: 'flex', borderRadius: '9px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.09)' }}>
              <input
                ...
                style={{ width: '140px', padding: '13px 14px', background: '#18181B', border: 'none', color: '#F8FAFC', fontSize: '14px', fontFamily: '"JetBrains Mono","Fira Code",monospace', letterSpacing: '3px', fontWeight: 600, outline: 'none' }}
              />
              <button onClick={join} style={{ padding: '13px 18px', background: '#059669', border: 'none', borderLeft: '1px solid rgba(255,255,255,0.08)', color: '#ffffff', cursor: 'pointer', fontSize: '14px', fontWeight: 600, fontFamily: 'Inter,sans-serif', display: 'flex', alignItems: 'center', gap: '5px', transition: 'background 0.15s' }}
        ...
        <div className="hero-right" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '10px', animation: 'fadeIn 0.7s ease 0.15s both' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '7px 13px', borderRadius: '8px', background: '#18181B', border: '1px solid rgba(255,255,255,0.07)', animation: `floatUp ${3 + i}s ease-in-out ${chip.delay} infinite` }}>
      ...
      <div style={{ textAlign: 'center', paddingBottom: '24px', color: '#374151', fontSize: '12px', animation: 'pulse 3s infinite' }}>scroll to explore</div>
    </div>
  )
}
...
function Features() {
  const list = [
    ...
    { Icon: Ico.Play,  color: '#10B981', title: 'Run code together', desc: 'Hit Run and the output appears for everyone in the room. Supports JS, Python, Java, C++, Go, Rust, C#, PHP, Ruby and more.' },
  ...
  return (
    <div id="features" style={{ background: '#111115', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '96px 32px' }}>
        <div style={{ textAlign: 'center', marginBottom: '60px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-1.5px', color: '#F8FAFC', marginBottom: '14px' }}>Everything you need to code together</h2>
          <p style={{ color: '#6B7280', fontSize: '16px', lineHeight: 1.7, maxWidth: '500px', margin: '0 auto' }}>No fluff. No pairing extensions. Just open a room and start writing.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
          {list.map((item) => (
            <div key={item.title} className="ce-card" style={{ background: '#09090B', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '26px' }}>
              <div style={{ width: 42, height: 42, borderRadius: '10px', background: `${item.color}15`, border: `1px solid ${item.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
    ...
    <div id="how">
      <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '96px 32px' }}>
        <div style={{ textAlign: 'center', marginBottom: '60px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-1.5px', color: '#F8FAFC' }}>Three steps. Thirty seconds.</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
          {steps.map((s) => (
            <div key={s.n} className="ce-step-card" style={{ background: '#0F0F11', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '28px' }}>
              <div style={{ width: 36, height: 36, borderRadius: '9px', background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#818CF8', fontWeight: 800, fontSize: '15px', marginBottom: '16px', fontFamily: '"JetBrains Mono","Fira Code",monospace' }}>
                {s.n}
              </div>
...
function FAQ() {
  ...
  const items = [
    ...
    { q: 'What languages can I run?', a: 'Code execution is supported for JavaScript, Python, Java, C, and C++. The editor supports syntax highlighting for 13 languages — including TypeScript, Go, Rust, C#, F#, PHP, Ruby, Haskell, and plain text.' },
  ...
  return (
    <div id="faq" style={{ background: '#111115', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '96px 32px' }}>
        ...
        {items.map((item, i) => (
          <div key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <button className="ce-faq-btn" onClick={() => setOpen(open === i ? null : i)} style={{ width: '100%', background: 'none', border: 'none', color: '#F8FAFC', fontFamily: 'Inter,sans-serif', fontSize: '15px', fontWeight: 600, textAlign: 'left', padding: '20px 0', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
              <span>{item.q}</span>
            ...
            {open === i && (
              <p style={{ color: '#6B7280', fontSize: '14.5px', lineHeight: 1.75, paddingBottom: '22px', animation: 'fadeIn 0.15s ease' }}>{item.a}</p>
            )}
          </div>
        ))}
      </div>
...
const ALL_NAMES = ['Alex','Maya','Ravi','Priya','Jordan','Sam','Wei','Noah','Chris','Lee','Zara','Jin']

function makePeer(name, i, total) {
  const angle = (i / total) * Math.PI * 2 + Math.random() * 0.4
  return { name, angle, dist: 46 + Math.random() * 14, vAngle: (Math.random() < 0.5 ? 1 : -1) * (0.0015 + Math.random() * 0.001), alpha: 1, state: 'stable', x: 0, y: 0 }
}
...
function NetworkViz() {
  ...
  useEffect(() => {
      ...
      return [
        {
          // Room 1 — 2 people
          fx: 0.25, fy: 0.38,
          x: w * 0.25, y: h * 0.38, r: 8,
          peers: [ makePeer('Alex', 0, 2), makePeer('Maya', 1, 2) ],
        },
        {
          // Room 2 — 3 people
          fx: 0.73, fy: 0.32,
          x: w * 0.73, y: h * 0.32, r: 8,
          peers: [ makePeer('Ravi', 0, 3), makePeer('Priya', 1, 3), makePeer('Jordan', 2, 3) ],
        },
        {
          // Room 3 — 5 people
          fx: 0.48, fy: 0.75,
          x: w * 0.48, y: h * 0.75, r: 8,
          peers: [ makePeer('Sam', 0, 5), makePeer('Wei', 1, 5), makePeer('Noah', 2, 5), makePeer('Chris', 3, 5), makePeer('Lee', 4, 5) ],
        },
      ]
    }
    ...
    function spawnPacket() {
      ...
      const stable = room.peers.filter(p => p.state === 'stable' && p.alpha > 0.8)
      if (!stable.length) return
      const peer = stable[Math.floor(Math.random() * stable.length)]
      packets.push({ room, peer, toRoom: Math.random() < 0.5, t: 0, speed: 0.016 + Math.random() * 0.009 })
    }
    const spawnTimer = setInterval(() => { if (Math.random() < 0.75) spawnPacket() }, 680)
    ...
    function triggerJoinLeave() {
      ...
      const checkGone = setInterval(() => {
        if (leaving.alpha <= 0.04) {
          ...
          leaving.name  = ALL_NAMES[Math.floor(Math.random() * ALL_NAMES.length)]
    ...
    const joinTimer = setInterval(triggerJoinLeave, 3200 + Math.random() * 1400)

    const highlight = { peer: null, until: 0 }
    function triggerHighlight() {
      const room = rooms[Math.floor(Math.random() * rooms.length)]
      const stable = room.peers.filter(p => p.state === 'stable' && p.alpha > 0.7)
      if (!stable.length) return
    ...
    function onResize() {
      ...
      stateRef.current.rooms.forEach(room => { room.x = w * room.fx; room.y = h * room.fy })
    ...
    function step() {
      ...
      for (const room of rs) {
        for (const peer of room.peers) {
          ...
          if      (peer.state === 'leaving') peer.alpha = Math.max(0,   peer.alpha - 0.014)
          else if (peer.state === 'joining') { peer.alpha = Math.min(1, peer.alpha + 0.014); if (peer.alpha >= 1) peer.state = 'stable' }
        }
      }

      for (let p = ps.length - 1; p >= 0; p--) {
        const pl = ps[p]; pl.r += 0.85; pl.op *= 0.966
        if (pl.op < 0.02) { ps.splice(p, 1); continue }
        ctx.beginPath(); ctx.arc(pl.x, pl.y, pl.r, 0, Math.PI * 2)
        ctx.strokeStyle = `rgba(99,102,241,${pl.op})`; ctx.lineWidth = 1.2; ctx.stroke()
      }

      for (const room of rs) {
        for (const peer of room.peers) {
          if (peer.alpha < 0.05) continue
          ctx.beginPath(); ctx.moveTo(room.x, room.y); ctx.lineTo(peer.x, peer.y)
          ctx.strokeStyle = `rgba(129,140,248,${peer.alpha * 0.32})`; ctx.lineWidth = 1; ctx.stroke()
        }
      }

      for (let p = pk.length - 1; p >= 0; p--) {
        const pkt = pk[p]; pkt.t += pkt.speed
        if (pkt.t >= 1 || pkt.peer.alpha < 0.3) { pk.splice(p, 1); continue }
        ...
        ctx.shadowColor = 'rgba(99,102,241,0.65)'; ctx.shadowBlur = 6
        ctx.fill(); ctx.shadowBlur = 0
      }

      for (const room of rs) {
        ctx.beginPath(); ctx.arc(room.x, room.y, room.r + 6, 0, Math.PI * 2)
        ctx.fillStyle = 'rgba(99,102,241,0.14)'; ctx.fill()
        ctx.beginPath(); ctx.arc(room.x, room.y, room.r, 0, Math.PI * 2)
        ctx.fillStyle = '#818CF8'; ctx.fill()
        ctx.fillStyle = 'rgba(13,15,26,0.88)'
        ctx.beginPath(); ctx.moveTo(room.x, room.y - 4.2); ctx.lineTo(room.x + 4, room.y - 0.5); ctx.lineTo(room.x - 4, room.y - 0.5); ctx.closePath(); ctx.fill()
        ...
        for (const peer of room.peers) {
          ...
          if (peer === highlight.peer && performance.now() < highlight.until) {
            ctx.beginPath(); ctx.arc(peer.x, peer.y, 7, 0, Math.PI * 2)
            ctx.fillStyle = 'rgba(255,255,255,0.06)'; ctx.fill()
          }
          ctx.beginPath(); ctx.arc(peer.x, peer.y, 4, 0, Math.PI * 2)
          ctx.fillStyle = `rgba(228,228,231,${peer.alpha * 0.88})`; ctx.fill()
        }
      }
      ...
      if (tooltip) {
        ...
        if (p && p.alpha > 0.6 && performance.now() < highlight.until) {
          ...
          tooltip.style.left = p.x + 'px'; tooltip.style.top = (p.y - 16) + 'px'; tooltip.style.opacity = '1'
    ...
    return () => {
      ...
      clearInterval(spawnTimer); clearInterval(pulseTimer); clearInterval(joinTimer); clearInterval(hlTimer)
      window.removeEventListener('resize', onResize)
    }
  }, [])

  return (
    <div ref={wrapRef} style={{ position: 'relative', width: '100%', height: '100%', minHeight: '220px', cursor: 'default' }}>
      <canvas ref={canvasRef} style={{ display: 'block', width: '100%', height: '100%' }} />
      <div ref={tooltipRef} style={{ position: 'absolute', transform: 'translate(-50%, -100%)', padding: '4px 10px', borderRadius: '6px', background: '#18181B', border: '1px solid rgba(255,255,255,0.1)', color: '#F8FAFC', fontSize: '11px', fontWeight: 600, fontFamily: 'Inter,sans-serif', whiteSpace: 'nowrap', pointerEvents: 'none', opacity: 0, transition: 'opacity 0.15s', zIndex: 5 }} />
    </div>
  )
}
...
function CTA({ onOpen }) {
  return (
    <div style={{ padding: '96px 32px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', background: 'rgba(9,9,11,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', overflow: 'hidden', display: 'flex', alignItems: 'stretch', flexWrap: 'wrap', boxShadow: '0 24px 60px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)' }}>

        {/* LEFT — glass viz panel */}
        <div style={{ flex: '0 0 auto', width: '320px', minHeight: '320px', position: 'relative', display: 'flex', flexDirection: 'column', background: 'linear-gradient(145deg, rgba(99,102,241,0.1) 0%, rgba(9,9,11,0.6) 50%, rgba(99,102,241,0.06) 100%)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderRight: '1px solid rgba(99,102,241,0.14)' }}>
          {/* Top edge glass highlight */}
          <div style={{ position: 'absolute', top: 0, left: '10%', right: '10%', height: '1px', background: 'linear-gradient(90deg, transparent, rgba(99,102,241,0.35), transparent)', zIndex: 1 }} />
          <div style={{ flex: 1, minHeight: 0 }}>
            <NetworkViz />
          </div>
          {/* Legend */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '10px 12px', borderTop: '1px solid rgba(99,102,241,0.1)' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <span style={{ width: 8, height: 8, borderRadius: '3px', background: '#818CF8' }} />
              <span style={{ color: '#7C8294', fontSize: '11px', fontFamily: 'Inter,sans-serif' }}>Room</span>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#E4E4E7' }} />
              <span style={{ color: '#7C8294', fontSize: '11px', fontFamily: 'Inter,sans-serif' }}>Collaborators</span>
            </span>
          </div>
        </div>

        {/* RIGHT — copy + buttons */}
        <div style={{ flex: 1, minWidth: '280px', textAlign: 'left', padding: 'clamp(36px,5vw,52px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2 style={{ fontSize: 'clamp(26px, 4vw, 38px)', fontWeight: 900, letterSpacing: '-1.5px', lineHeight: 1.1, color: '#F8FAFC', marginBottom: '14px' }}>
            Open a room.<br />
            <span style={{ color: '#52525B' }}>Start coding together.</span>
          </h2>
          <p style={{ color: '#52525B', fontSize: '15px', lineHeight: 1.7, maxWidth: '340px', marginBottom: '30px' }}>
            No account. No setup. Share a 6-character code and your team shows up — live, like this.
          </p>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '24px' }}>
            <button className="ce-btn-primary" onClick={onOpen} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '14px 28px', background: '#6366F1', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '15px', fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter,sans-serif' }}>
              Create a room — it's free
            </button>
            <a href="https://github.com/user1-prajwal/collab-editor" target="_blank" rel="noreferrer" className="ce-btn-ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '14px 22px', background: 'transparent', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '10px', color: '#6B7280', fontSize: '15px', fontWeight: 600, textDecoration: 'none' }}>
    ...
    <footer style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '48px 32px 28px' }}>
      <div style={{ maxWidth: '1160px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '36px', marginBottom: '36px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '10px' }}>
              <div style={{ width: 24, height: 24, borderRadius: '6px', background: '#6366F1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Ico.Bolt s={12} c="#fff" /></div>
              <span style={{ fontWeight: 700, fontSize: '14px', color: '#F8FAFC' }}>Collab Editor</span>
            </div>
            <p style={{ color: '#3F3F46', fontSize: '13px', lineHeight: 1.7, marginBottom: '14px' }}>Free, open-source real-time collaborative code editor.</p>
            <div style={{ display: 'flex', gap: '7px' }}>
              {[{ href: 'https://github.com/user1-prajwal', Ic: Ico.Github }, { href: 'https://www.linkedin.com/in/user1-prajwal451/', Ic: Ico.In }].map((s, i) => (
                <a key={i} href={s.href} target="_blank" rel="noreferrer" className="ce-social" style={{ width: 30, height: 30, borderRadius: '7px', background: '#18181B', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B7280', textDecoration: 'none' }}>
                  <s.Ic s={14} />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            {['Real-time sync','Live cursors','Code execution','Built-in chat','Multi-file','Import & Export'].map(f => (
              <div key={f} style={{ color: '#3F3F46', fontSize: '13px', marginBottom: '8px' }}>{f}</div>
            ))}
          </div>
          <div>
            <h4 style={{ color: '#F8FAFC', fontSize: '13px', fontWeight: 600, marginBottom: '12px' }}>Languages</h4>
            {['JavaScript','TypeScript','Python','Java','C / C++','Go','Rust','C# / F#','PHP','Ruby','Haskell'].map(l => (
              <div key={l} style={{ color: '#3F3F46', fontSize: '13px', marginBottom: '8px' }}>{l}</div>
            ))}
          </div>
          <div>
            <h4 style={{ color: '#F8FAFC', fontSize: '13px', fontWeight: 600, marginBottom: '12px' }}>Links</h4>
            <button onClick={onOpen} style={{ display: 'block', background: 'none', border: 'none', color: '#3F3F46', fontSize: '13px', cursor: 'pointer', padding: 0, marginBottom: '8px', fontFamily: 'Inter,sans-serif', textAlign: 'left' }}>Create Room</button>
            {[{ href: 'https://github.com/user1-prajwal/collab-editor', label: 'GitHub' }, { href: 'https://github.com/user1-prajwal/collab-editor/issues', label: 'Report a Bug' }].map(l => (
              <a key={l.label} href={l.href} target="_blank" rel="noreferrer" style={{ display: 'block', color: '#3F3F46', fontSize: '13px', textDecoration: 'none', marginBottom: '8px', transition: 'color 0.15s' }}
                onMouseOver={e => e.currentTarget.style.color = '#F8FAFC'}
                onMouseOut={e => e.currentTarget.style.color = '#3F3F46'}>{l.label}</a>
            ))}
          </div>
        </div>
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '20px', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
          <span style={{ color: '#3F3F46', fontSize: '12px' }}>© 2025 Collab Editor · Free forever · No data stored · MIT License</span>
          <span style={{ color: '#3F3F46', fontSize: '12px' }}>React + Socket.io + Monaco Editor</span>
        </div>
      </div>
    </footer>
  )
